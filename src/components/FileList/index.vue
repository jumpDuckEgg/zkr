<template>
    <div>
        <el-row type="flex" justify="start" style="flex-wrap:wrap" v-if="showListVisible">
        ...
        <div v-if="showDocument">
            ...
            <div v-loading='pdfloading'>
                <div class="documentBox">
                    <pdf style="padding:20px;" :src="pdfUrl" :page="page" @num-pages="numPages = $event" @loaded="loaded"></pdf>
                </div>
                <el-row class="article-bar" type="flex" justify="space-around">
                    ...
                    <el-col :span="4" class="article-bar__third-column">
                        {{page}}/{{numPages}}
                        <!-- <el-input size="small" min="1" :max="numPages" class="article-bar__page-input" @keyup.native.enter="changeCurrentPage" v-model.number="pageInput" type="number"></el-input>/{{numPages}} -->
                    </el-col>
                </el-row>
            </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    showFile(id, name) {
      ...
      const isImage = /^(?:jpeg|png|jpg)$/i.test(typeTemp[typeTemp.length - 1]);
      const isPdf = /^(?:pdf)$/i.test(typeTemp[typeTemp.length - 1]);
      if (isImage) {
        this.imgUrl = "http://182.61.47.252:9998/zkr/page/file/get?id=" + id;
        this.showVisible = true;
        this.showListVisible = false;
        return true;
      }
      if (isPdf) {
        this.pdfUrl = "http://182.61.47.252:9998/zkr/page/file/get?id=" + id;
